map_nearest_state = function(name){
...
leaflet() %>%
  ...
  addCircles(data = st_transform(vor, 4326),
             radius = 1, color = ~colorFactor("viridis", state_name)(state_name)) %>%
  addPolygons(data = st_transform(soi, 4326),
              fillColor = "transparent",
              color = "black") %>%
  addPolygons(data = st_transform(adjoining, 4326),
              fillColor = ~colorFactor("viridis", state_name)(state_name), col = "transparent") %>%
